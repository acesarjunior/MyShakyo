...
var TissaranaPali = "Tissarana\n\nBuddham saranam gachami\nDhammam saranam gachami\nSangham saranam gachami";

class TissaranaScreen extends StatefulWidget {
  const TissaranaScreen({super.key});

  @override
  _TissaranaScreenState createState() => _TissaranaScreenState();
}

class _TissaranaScreenState extends State<TissaranaScreen> {
  late stt.SpeechToText _speech;
  bool _isListening = false;
  String _text = TissaranaPali;
  String _lastWords = '';
  String selectedLanguage = 'Pali';
  List<String> words = [];
            ...
            LanguageSelectorDropdown(
              ...
              onLanguageChanged: (language) {
                setState(() {
                  ...
                  _text = language == 'Pali' ? TissaranaPali : TissaranaPali ;
                  words = _text.split(RegExp(r'\s+|\n')).where((s) => s.isNotEmpty).toList();
                  wordsForComparison = _cleanTextForComparison(_text).split(' ').where((s) => s.isNotEmpty).toList();
                  currentWordIndex = 0;
                });
